...
export function coachMoveWithoutEval(move, game, gameBeforeMove) {
  ...
  const posAnalysis = analyzePosition(null, game);
  if (posAnalysis) {
    // Material commentary — only when there's a significant difference
    if (posAnalysis.materialDiff >= 5) {
      messages.push("White is up significantly in material.");
    } else if (posAnalysis.materialDiff <= -5) {
      messages.push("Black is up significantly in material.");
    }

    // Game phase — only mention once when transitioning
    if (posAnalysis.moveCount > 30 && (posAnalysis.whiteMaterial + posAnalysis.blackMaterial) < 15) {
      messages.push("Endgame — activate your king and push passed pawns.");
    }
  }
