...
function getPhaseStrategy(phase, position, playerColor) {
  if (phase === "opening") {
    const tips = [];
    const playerDeveloped =
      playerColor === "w" ? position.whiteDeveloped : position.blackDeveloped;
    if (playerDeveloped < 2) {
      tips.push("focus on developing your knights and bishops before pushing pawns");
    }
    tips.push("control the center, develop your pieces, and get castled in the next few moves");
    return tips.join(" — ");
  }
  if (phase === "middlegame") {
    return pickRandom([
      "look for tactics like forks, pins, and skewers, and try to create threats against your opponent",
      "improve your worst-placed piece and look for ways to coordinate an attack",
      "identify your opponent's weakest piece or square and pile pressure on it",
      "if you have an advantage, look for ways to convert it — if not, solidify your position",
    ]);
  }
  return pickRandom([
    "activate your king as a fighting piece and push your passed pawns toward promotion",
    "simplify when you're ahead by trading pieces, and complicate when you're behind",
    "use your king aggressively — in the endgame it's one of your strongest pieces",
  ]);
}
...
export function generateInstantFeedback({
  ...
  if (blunder) {
    ...
    parts.push(pickRandom([
      "Before each move, ask yourself: 'can they take anything for free after this?'",
      "Always double-check that your move doesn't leave a piece undefended.",
      "A good habit is to scan all of your opponent's captures before finalizing your move.",
    ]));
    return parts.join(" ");
  }
  ...
  if (playerPlayedBest) {
    ...
    // Suggest the engine's best move if available
    if (beforeEval?.bestMove && gameBefore) {
      try {
        ...
        if (bestMove) {
          ...
          if (bestAchievements.length > 0) {
            parts.push(
              `The engine preferred ${bestMove.san}, which ${joinAchievements(bestAchievements)}.`
            );
          } else {
            parts.push(`The engine's top choice was ${bestMove.san}.`);
          }
        }
      } catch {
        // ignore
  ...
  if (position) {
    const strategy = getPhaseStrategy(phase, position, playerColor);
