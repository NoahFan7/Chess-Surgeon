...
export function coachMoveWithoutEval(move, game, gameBeforeMove) {
  ...
  if (move.captured) {
    ...
    const capturedValue = PIECE_VALUES[move.captured] || 0;
    if (capturedValue >= 5) {
      messages.push(`${moverColor} captures a ${capturedName} with ${move.san} — that's a major piece! Big material swing.`);
    } else if (capturedValue >= 3) {
      messages.push(`${moverColor} captures a ${capturedName} with ${move.san}. Good material gain.`);
    } else if (capturedValue > 0) {
      messages.push(`${moverColor} takes a ${capturedName} with ${move.san}.`);
    }
    return messages.join(" ");
  }
  ...
  messages.push("I'm analyzing this with the engine for deeper evaluation...");
