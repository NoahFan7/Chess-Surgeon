...
export function coachMoveWithoutEval(move, game, gameBeforeMove) {
  ...
  // Describe the move
  messages.push(`${moverColor} played ${move.san}.`);
  ...
  if (move.captured) {
    ...
    const capturedValue = PIECE_VALUES[move.captured] || 0;
    if (capturedValue >= 5) {
      messages.push(`That captures a ${capturedName} — a major piece! That's significant material.`);
    } else if (capturedValue >= 3) {
      messages.push(`That captures a ${capturedName} — a minor piece. Good material gain.`);
    } else if (capturedValue > 0) {
      messages.push(`That takes a ${capturedName}.`);
    }
  }

  // Check if it's a check
  if (move.san && move.san.includes("+")) {
    messages.push("This move gives check — the opponent must respond to the threat!");
  }

  // Checkmate
  if (move.san && move.san.includes("#")) {
    messages.push("Checkmate! The game is over!");
    return messages.join(" ");
  }
  ...
  if (move.san && move.san.includes("O-O")) {
    if (move.san.includes("O-O-O")) {
      messages.push("Queenside castling — good for king safety and activates the rook.");
    } else {
      messages.push("Kingside castling — a solid move that secures the king and develops the rook.");
    }
  }
  ...
  if (move.promotion) {
    ...
    messages.push(`A pawn promotion to a ${promoName}! That's a big material upgrade.`);
  }

  // Position-based coaching using the position AFTER the move
  const posAnalysis = analyzePosition(null, game);
  if (posAnalysis) {
    // Material commentary
    if (posAnalysis.materialDiff >= 5) {
      messages.push("White is up significantly in material — White should look to trade pieces and simplify.");
    } else if (posAnalysis.materialDiff <= -5) {
      messages.push("Black is up significantly in material — Black should look to trade pieces and simplify.");
    } else if (posAnalysis.materialDiff >= 2) {
      messages.push("White has a small material lead.");
    } else if (posAnalysis.materialDiff <= -2) {
      messages.push("Black has a small material lead.");
    }

    // Development commentary (early game)
    if (posAnalysis.moveCount < 20) {
      if (posAnalysis.turn === "w" && posAnalysis.whiteDeveloped < 3) {
        messages.push("In the opening, White should focus on developing pieces and controlling the center.");
      } else if (posAnalysis.turn === "b" && posAnalysis.blackDeveloped < 3) {
        messages.push("In the opening, Black should focus on developing pieces and controlling the center.");
      }
    }

    // Game phase
    if (posAnalysis.moveCount > 30 && (posAnalysis.whiteMaterial + posAnalysis.blackMaterial) < 20) {
      messages.push("We're in the endgame now — activate your king and push passed pawns.");
    }
  }

  messages.push("Let me analyze this position with the engine for more detailed feedback...");
