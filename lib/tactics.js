...
export const TACTICS = {
  ...
  deflection: {
    ...
    examples: [
      {
        fen: "6k1/5ppp/5n2/8/8/8/8/3QR1K1 w - - 0 1",
        moves: ["Qd8+", "Ne8", "Rxe8#"],
        arrows: [{ from: "d1", to: "d8" }, { from: "e1", to: "e8" }],
        explanations: [
          "The black knight on f6 is guarding the e8 square — if it moves, White can play Re8 for back-rank mate. White plays Qd8+, giving check along the 8th rank. The king on g8 is in check and must respond. The knight is the only piece that can block — it jumps to e8 to interpose.",
          "The knight has been deflected from f6 to e8! It was guarding the e8 square, but now it's ON e8 — it had to go there to block the check. This is the deflection: the defender was forced to abandon its defensive post.",
          "White plays Rxe8#, checkmate! The rook captures the knight on e8 with check. The king can't escape — pawns on f7, g7, and h7 block all escape squares, and the rook controls the entire 8th rank. The knight was deflected from guarding e8 to blocking on e8, where it could be captured.",
        ],
        result: "Checkmate! White deflected the knight from f6 with a queen check, forcing it to block on e8 where it was captured for mate.",
      },
      {
        fen: "6k1/4bppp/8/8/8/8/5PPP/3QR1K1 w - - 0 1",
        moves: ["Qd8", "Bxd8", "Re8#"],
        arrows: [{ from: "d1", to: "d8" }, { from: "e1", to: "e8" }],
        explanations: [
          "This time a bishop on e7 is the defender — it guards the f8 square, preventing White's rook from delivering mate there. White plays Qd8, offering the queen as a sacrifice. The queen attacks the bishop, and the bishop must capture or be lost.",
          "The bishop captures the queen with Bxd8. The deflection worked — the bishop has been dragged from e7 to d8, and the critical f8 square is now undefended. Crucially, the bishop on d8 can't defend e8 or f8 — bishops move diagonally, not along ranks.",
          "White plays Re8#, checkmate! The rook slides from e1 to e8, checking the king. The king can't escape — pawns on f7, g7, h7 block all escape squares, and the rook controls the entire 8th rank. The bishop on d8 can't help — it can only move diagonally and can't interpose on the 8th rank.",
        ],
